...
function drawLogo(_xx, _yy, _scale) {
	...
	ctx.textAlign = "center";
	ctx.save()
	ctx.fillText("ROCKET", titlePos.x, titlePos.y);
	ctx.restore()
	ctx.save()
	ctx.font = "bold 118px Tahoma";
	ctx.fillText("LAUNCH", titlePos.x + 5, titlePos.y + 120);
	ctx.restore()
...
function drawTexts() {
	...
	ctx.textAlign = "center";
	ctx.fillText(dText + currentPolyTextToShow, pTextPos.x, pTextPos.y);
	ctx.globalAlpha = 1
...
function story32() {
	setStoryMessage(["Heyy, you really did it,", "finished the game:", " ", "ROCKETLAUNCH!!!", " ",
...
function story13() {
	setStoryMessage(["OKAY, let's roll up", "our sleeves and get", "those Rockyblocks back!", " ", "Remember:", "any game mode is fine,", "as long as you feel comfortable.", " ", "ROCKET LAUNCH!!"])
...
function sendRandomMessage() {
	...
	var randomMessages = ["Put them into the ship!", "Drag and drop them in!", "Double click to rotate!", "This is a fun one!", "ROCKET LAUNCH!!",
...
function saveData(item, data) {
	localStorage.setItem("rocketlaunch_" + item, data)
}

function loadData(item, _default) {
	return localStorage.getItem("rocketlaunch_" + item) || _default
}
